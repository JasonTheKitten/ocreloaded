    ...
    public void onMessage(NetworkMessage message, NetworkNode sender) {
        ...
        switch (message.name()) {
            ...
            case "keyboard.keyDown": {
                ...
                node.sendToReachable(new NetworkMessage("computer.checked_signal", null, "key_down", charCode, keyCode, playerName));
                break;
            }
            case "keyboard.keyUp": {
                // Player player = null;//(Player) message.arguments()[0];
                ...
                if (charCode != null) {
                    String playerName = "";//player.getName().getString();
                    node.sendToReachable(new NetworkMessage("computer.checked_signal", null, "key_up", (int) charCode, keyCode, playerName));
                }
                break;
            }
            case "keyboard.clipboard": {
                ...
                if (clipboard != null) {
                    String playerName = "";//player.getName().getString();
                    node.sendToReachable(new NetworkMessage("computer.checked_signal", null, "clipboard", clipboard, playerName));
                }
            }
            default:
                break;
